        ...
        <div class="panel_body">
            <form action="">
                <div class="field clearfix">
                    <label for="" class="required">用户名：</label><input type="text" placeholder="请输入正确的手机号/帐号" name="username" v-model="username">
                </div>
                <div class="field clearfix">
                    <label for="" class="required">密码：</label><input type="password" placeholder="请输入正确的密码" name="password" v-model="password">
                </div>
                <div class="field clearfix">
                  <router-link tag="a" to="/register" v-bind:class="style.apply">没有帐号？<span :class="style.orange">免费申请</span></router-link>
...
export default {
  ...
  methods: {
    userLogin() {
      this.$http.get("../../static/data/userData.json").then(
        res => {
          var data = res.body;
          if (data.code == 1) {
            if (
